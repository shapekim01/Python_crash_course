...
```python
# range() 함수 사용
for value in range(1, 5): # == range(0, 5)
    print(value)
# 출력값 = 1 2 3 4
...
### 4.4.1 슬라이스 만들기
- 슬라이스를 만들 때는 그 범위의 인덱스를 지정하면 됨
```python
# 슬라이스 사용
players = ['charles', 'martina', 'michael', 'florence', 'eli']
print(players[0:3])    # ['charles', 'martina', 'michael']
print(players[:4]) # 0부터, # ['charles', 'martina', 'michael', 'florence', 'eli']
print(players[2:]) # 인덱스 2부터, # ['michael', 'florence', 'eli']
print(players[-3:]) # 마지막 세 명의 플레이어 # ['michael', 'florence', 'eli']

# 4.4.2 슬라이스 순회
for player in players[:3]:    # 처음 세 명의 이름만 순회
    print(player)
...
- Tab을 눌렀을 때 이를 일정한 공백으로 변환하는 기능 활성화
- 대부분의 파이썬 프로그래머는 각 행의 길이가 **80자 미만**이어야 한다고 권함.
