...
# Hello
```
- `def`는 함수를 정의하겠다는 뜻인 키워드이다. 이를 **함수 정의** 라 부름.
- 함수 정의 다음에 들여 쓴 행은 모두 함수 **바디** 임.
- 두 번째 행의 텍스트는 **독스트링** 으로 함수가 수행하는 작업을 설명하는 주석임임.
...
```
이 방법은 점 표기법을 사용하지 않고 함수 이름을 바로 호출하기 때문에 권장하지 않음.
